...
const classes = {
    ...
    title: cn('nx-flex nx-font-semibold nx-items-start nx-gap-2 nx-p-4 nx-text-gray-700 hover:nx-text-gray-900')
}
...
export function Card({
    ...
    if (image) {
        return (
            ...
            >
                {children}
                <span className={cn(classes.title, 'dark:nx-text-gray-300 dark:hover:nx-text-gray-100')}>
                    {icon}
...
function _Cards({ children, num = 3, className, style, ...props }: { num?: number } & ComponentProps<'div'>) {
    return (
        <div
